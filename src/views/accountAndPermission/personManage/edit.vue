                ...
                <el-row :gutter="20">
                    ...
                    <el-col :span="8">
                        <el-form-item label="入队时间：" prop="joinDate">
                            <el-date-picker type="date" value-format="yyyy-MM-dd" placeholder="请选择入队时间" v-model="businessForm.joinDate" style="width: 100%;"></el-date-picker>
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
...
<script>
    ...
    export default {
        ...
        created() {
            this.getAllList(this.getDetails); // 获取基础下拉列表
        },

        methods: {
            // 查询人员详情
            getDetails() {
                getStaffDetail({staffId: this.personForm.staffId}).then(res => {
                    if(res.data.code == 200) {
                        let resData = res.data.data;
                        this.imgUrl = resData.photo;
                        this.personForm.name = resData.staffName;
                        this.personForm.sex = resData.gender.toString();
                        ...
                        this.personForm.status = resData.status.toString();
                    ...
                    if (valid) {
                        this.$refs.businessForm.validate((valid) => {
                            if (valid) {
                                ...
                                saveStaff({
                                    // 人员信息
                                    staffId: this.personForm.staffId,
